...
constexpr const int ZShape::COORDINATES[4][2];
constexpr const int ZShape::INITIAL_COORDINATES[3][2];  
      
ZShape::ZShape() : ZShape(Square()){}
ZShape::ZShape(Square example) : ZShape(example,0){}
ZShape::ZShape(Square example, int state) : Shape() {
...
ZShape::ZShape(const ZShape& other):Shape(other){
    state=other.state;
}


void ZShape::rotateShape(vector<Square*> &cloneSquares) {
        state++;
        state=state%NUMBER_OF_STATES;
        int multiplier=1;
