...
constexpr const int LShape::COORDINATES[4][4][2];
constexpr const int LShape::INITIAL_COORDINATES[3][2];

LShape::LShape() : LShape(Square()) {
}

LShape::LShape(Square example) : LShape(example, 0) {
}

LShape::LShape(Square example, int state) : Shape() {
...
LShape::LShape(const LShape& other) : Shape(other) {
    state = other.state;
}

void LShape::rotateShape(vector<Square*> &cloneSquares) {
    state++;
    state = state % NUMBER_OF_STATES;
